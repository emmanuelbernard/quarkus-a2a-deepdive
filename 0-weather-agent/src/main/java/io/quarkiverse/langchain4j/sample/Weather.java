...
public class Weather {
    ...
    @Tool("Get weather alerts for a US state.")
    String getAlerts(@P("Two-letter US state code (e.g. CA, NY)") String state) {
        return formatAlerts(weatherClient.getAlerts(state));
    }

    @Tool("Get weather forecast for a location.")
    String getForecast(@P("Latitude of the location") double latitude,
                       @P("Longitude of the location") double longitude) {
